...
def collect_algolia(start_iso, end_iso, min_score, keywords, only_show_hn, resolve_urls):
    ...
    while True:
        params = {
            "query": "",  # we filter ourselves using keywords/title/url
            "tags": "story",
...
def main():
    ...
    if args.method == "algolia":
        kept, hits_total = collect_algolia(start_iso, end_iso, args.min_score, args.query, args.only_show_hn, args.resolve_urls)
    else:
        kept, hits_total = collect_firebase(start_iso, end_iso, args.min_score, args.query, args.only_show_hn, args.resolve_urls)
    ...
    # Save CSV
    df = pd.DataFrame(kept, columns=["hn_id","time_utc","title","url","resolved_url","score","descendants","is_show_hn","author"])
    ...
    if not df.empty:
        ...
        summ.append("No posts matched the filters. Consider lowering --min_score or widening dates.")
