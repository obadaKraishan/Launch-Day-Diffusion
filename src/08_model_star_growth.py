...
Outputs:
  ...
  - outputs/figures/08_enet_coefficients.png
  - outputs/figures/08_gb_permutation_importance.png
"""
...
def build_preprocessor(cat_cols, bool_cols, num_cols):
    ...
    booleans = Pipeline([
        ("imp", SimpleImputer(strategy="most_frequent"))  # keep 0/1
    ])
    nums = Pipeline([
...
def model_one_target(df, target_col, random_state=42):
    # Select rows with target present
    ...
    # keep id columns for output merge later
    key_cols = [k for k in ["hn_id","owner","repo"] if k in d.columns]
    ids = d[key_cols].copy()

    # build preprocessor & models
    pre = build_preprocessor(cat_cols, bool_cols, num_cols)

    enet = Pipeline([
        ("pre", pre),
        ("mdl", ElasticNetCV(l1_ratio=[0.1,0.3,0.5,0.7,0.9], alphas=np.logspace(-3,2,20),
                             cv=5, n_jobs=None, max_iter=5000, random_state=random_state))
    ])
    ...
    # Permutation importance for GBR on the pipeline (robust to preprocessing)
    ...
    # plots
    plot_top(coef_abs, f"Elastic Net | {target_col} | |coef| (top 20)", FIG_COEF.replace(".png", f"_{target_col}.png"))
    plot_top(imp, f"GradBoost | {target_col} | Permutation importance (top 20)", FIG_IMP.replace(".png", f"_{target_col}.png"))

    # bundle predictions table
    preds = ids_test.copy()
    ...
    # top features quick text
    summary["enet_top10"] = coef_abs.head(10).to_dict()
    summary["gbr_top10"]  = imp.head(10).to_dict()
...
def main():
    ...
    # Run for three horizons (primary = 48h)
    ...
    # Save predictions (wide merge by keys)
    if all_preds:
        out = all_preds[0]
